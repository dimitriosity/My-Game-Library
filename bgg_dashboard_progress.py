...
def load_full_data_with_progress():
    detailed = []
    ...
    for i, game in enumerate(games):
        ...
        status.text(f"Loading {i + 1} of {len(games)}: {game['Title']}")
...
with st.spinner("Fetching game data from BGG..."):
    df = load_full_data_with_progress()
